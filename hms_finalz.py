...
def create_login_box():
    ...
    # Logo Image
    logo_image = Image.open("D:\DELL E7450\Desktop\Final Project\HMS (Final)\logo.png")
...
def create_admin_login_box():
    ...
    # Logo Image
    logo_image = Image.open("D:\DELL E7450\Desktop\Final Project\HMS (Final)\logo.png")  # Replace with the path to your logo
...
def create_admin_window(cursor, mydb):
    ...
    def create_search_window():
        ...
        search_logo_path = "D:\DELL E7450\Desktop\Final Project\HMS (Final)\search.png"
        ...
        # Back Button
        back_button_path = "D:\DELL E7450\Desktop\Final Project\HMS (Final)\back.png"  # Provide the path to your back button image
...
def show_appointment_summary(appointment_info):
    ...
    def update_appointment():
        ...
        for index, (key, value) in enumerate(appointment_info.items(), start=1):
            if key == "Reason":
                # Use Radiobuttons for the "Reason" field
                label = tk.Label(update_window, text=f"{key}:", font=("Arial", 12))
                label.grid(row=index, column=0, pady=5, padx=(10, 0), sticky="w")

                reason_var = tk.StringVar(value=value)

                checkup_radio = tk.Radiobutton(update_window, text="Checkup", variable=reason_var, value="Checkup", padx=0)
                checkup_radio.grid(row=index, column=1, pady=2, sticky="w")

                medication_radio = tk.Radiobutton(update_window, text="Medication", variable=reason_var, value="Medication", padx=0)
                medication_radio.grid(row=index, column=2, pady=2, sticky="w")

                updated_info[key] = reason_var
            else:
                # Use Entry widgets for other fields
                label = tk.Label(update_window, text=f"{key}:", font=("Arial", 12))
                ...
                entry.grid(row=index, column=1, pady=5, padx=10, sticky="w")
                updated_info[key] = entry

        # Function to update the appointment in the database
        def save_updated_appointment():
            global update_values
            for key, entry in updated_info.items():
                if isinstance(entry, tk.StringVar):
                    # For Radiobuttons (reason field)
                    update_values[key] = entry.get()
                else:
                    # For Entry widgets
                    update_values[key] = entry.get()
